...
logging.basicConfig(level=logging.INFO,
                    ...
                    handlers=[
                            logging.FileHandler("Logs/ria_logger_2020_1.log"),
...
class riaParser:
    ...
    def get_metadata(self, list_item) -> dict[str, Any] | datetime.datetime | Literal[False] | str:
        ...
        if text == "continue":
            return "continue"
        if date_time >= datetime.datetime(2020, 9, 1):
            if text:
                row = {
                    "id" : [id],
    ...
    def process(self):
        ...
        """
        start_date = datetime.datetime(year=2020, month=11, day=12, hour=17, minute=20, second=0)
        id = "0"

        if os.path.isfile("Data/ria_data_2020_1.csv"):
            main_df = pd.read_csv("Data/ria_data_2020_1.csv")
        else:
            table_init = {
                "id" : [1],
        ...
        try:
            ...
            logging.error(f"An error occured: {err}" , exc_info = True)
                ...
                main_df.to_csv(path_or_buf="Data/ria_data_2020_1.csv", index=False)
