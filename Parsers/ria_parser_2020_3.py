...
logging.basicConfig(level=logging.INFO,
                    ...
                    handlers=[
                            logging.FileHandler("Logs/ria_logger_2020_3.log"),
...
class riaParser:
    ...
    def get_metadata(self, list_item) -> dict[str, Any] | datetime.datetime | Literal[False] | str:
        ...
        if text == "continue":
            return "continue"
        if date_time >= datetime.datetime(2020, 1, 1):
            if text:
                row = {
                    "id" : [id],
    ...
    def process(self):
        ...
        """
        start_date = datetime.datetime(year=2020, month=3, day=31, hour=4, minute=31, second=0)
        id = "0"

        if os.path.isfile("Data/ria_data_2020_3.csv"):
            main_df = pd.read_csv("Data/ria_data_2020_3.csv")
        else:
            table_init = {
                "id" : [1],
        ...
        try:
            ...
            logging.error(f"An error occured: {err}" , exc_info = True)
                ...
                main_df.to_csv(path_or_buf="Data/ria_data_2020_3.csv", index=False)
