...
logging.basicConfig(level=logging.INFO,
                    ...
                    handlers=[
                            logging.FileHandler("Logs/tass_logger_2020_2.log"),
...
class tassConnector:
    ...
    def get_metadata(self, response: List, number: int) -> dict[str, Any] | Literal[False] | None | datetime.date | str:
        ...
        if current_date >= datetime.datetime(year=2020, month=5, day=1):
...
class NewsParser:
    ...
    def process (self):
        ...
        """
        # start_date =  
        start_date = datetime.datetime(year=2020, month=5, day=18, hour=13, minute=57, second=54)

        if os.path.isfile("Data/tass_data_2020_2.csv"):
            main_df = pd.read_csv("Data/tass_data_2020_2.csv")
        else:
            table_init = {
                "id" : [1],
        ...
        try:
            ...
            logging.error(f"An error occured: {err}" , exc_info = True)
                ...
                main_df.to_csv(path_or_buf="Data/tass_data_2020_2.csv", index=False)
