...
logging.basicConfig(level=logging.INFO,
                    ...
                    handlers=[
                            logging.FileHandler("Logs/tass_logger_2022.log"),
...
class tassConnector:
    ...
    def get_metadata(self, response: List, number: int) -> dict[str, Any] | Literal[False] | None | datetime.date | str:
        ...
        if current_date >= datetime.datetime(2022, 4, 22):
...
class NewsParser:
    ...
    def process (self):
        ...
        start_date =datetime.datetime(year=2022, month=5, day=21, hour=17, minute=10, second=15)

        if os.path.isfile("Data/tass_data_2022.csv"):
            main_df = pd.read_csv("Data/tass_data_2022.csv")
        else:
            table_init = {
                "id" : [1],
        ...
        try:
            ...
            logging.error(f"An error occured: {err}" , exc_info = True)
                ...
                main_df.to_csv(path_or_buf="Data/tass_data_2022.csv", index=False)
